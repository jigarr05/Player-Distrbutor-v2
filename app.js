...
// Add Players to Playing 

document.querySelector('.player-roaster').addEventListener('dblclick', function(ev) {
  if (ev.target.classList.contains('regular')) {
    const playerEl = ev.target;
    playerEl.classList.add('hidden');
    const playerName = ev.target.innerHTML;
    players.push(playerName);
    // console.log(players);
    playerNumber++;
    const listItem = document.createElement("li");
    listItem.classList.add('list');
    listItem.append(`${playerNumber}. ${playerName}`);
